...
```{r}
diff_new_jersey <- filter(table, post == 1 & state == 1)$mean_empft - filter(table, post == 0 & state == 1)$mean_empft

diff_penn <- filter(table, post == 1 & state == 0)$mean_empft - filter(table, post == 0 & state == 0)$mean_empft

diff_new_jersey - diff_penn
```
...
```{r}
DID1<-lm(empft~state*post,data=for_regression); summary(DID1)
```

We can also add covariates and estimate
```{r}
DID2<-lm(empft~state*post+nregs+hrsopen+d2+d3+d4,data=for_regression); summary(DID2)
```
