...
```{r}
iv_c <- ivreg(lnw ~ d*factor(lotcateg) + d*factor(year) | 
                z*factor(lotcateg) + z*factor(year) , data=lottery)
kable(tidy(iv_c), digits=4, align='c',caption="2SLS Estimation")
```

The 2SLS estimate is $0.449 \ (t=0.725, p=0.469)$. This value is statistically insignificant and far larger than the unconditional LATE.
